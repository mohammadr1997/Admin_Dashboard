...
useEffect(()=>{
 if(setSelectedBanner && data){
  const updatedData=data.map((banner:bannerType,index:number)=>({...banner,isActive:index===0 ? true : false}))
...
if(selectedBanners && setSelectedBanner){
  const updatedBanners=selectedBanners.filter((banner)=>banner.imageName!==name)
...
if(!selectedBanners || !setSelectedBanner) return
 const updatedBanner=selectedBanners.map((banner:bannerType)=>({...banner, isActive:name==banner.imageName}))
...
>
      ...
      <SelectContent className=''>
        <SelectGroup >
         {types.map((type,key:number)=>{
          return   <SelectItem className='cursor-pointer' key={key}  value={type}>{type}</SelectItem>
         })}
        ...
        <div className='  text-black text-center grid grid-cols-1 md:flex md:flex-row md:flex-wrap justify-center gap-10 mt-14  bg-[#189DAC] min-h-[44rem]'>{selectedBanners ? selectedBanners.map((banner:bannerType,key:number)=>{
          return <div className='mt-2 mb-2' key={key}>
            ...
            <CardContent className='w-[350px] grid grid-cols-1'>
              ...
              <Image className='w-[350px]  rounded-3xl mt-5 h-[350px] mx-auto cursor-pointer hover:scale-110 transition-all duration-700' src={banner.src} alt={banner.imageName} width={600} height={600}/>
              <CardTitle className='mt-2 text-left p-4 text-md lg:text-xl font-bold'> {banner.imageName}</CardTitle>
              <div className='grid grid-cols-3 gap-6 mb-4 mt-1'>
                  <span> <Dialog><DialogTrigger asChild>
                    <Button className='!text-black !font-bold !text-md !lg:text-xl !px-4 !py-2 !rounded-2xl !border-1 !border-gray cursor-pointer' variant='ghost'>Preview</Button></DialogTrigger>
                    <DialogContent>
                      <DialogTitle></DialogTitle>
                                        <Image className='w-[450px]h-[450px]' alt={banner.imageName} src={banner.src} width={800} height={800}/></DialogContent>
                    </Dialog></span>
                  <span>
                    {editBanner ? <Dialog>
                      <DialogTrigger asChild>
                         <Button onClick={()=>setEditBanner(banner)} className='!text-black !font-bold !text-md !lg:text-xl !px-4 !py-2 !rounded-2xl !border-1 !border-gray cursor-pointer' variant='ghost'>Edit</Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogTitle></DialogTitle>
                        <form className='text-md lg:text-lg font-bold grid grid-cols-1 gap-6'>
                          <div className='flex flex-row flex-nowrap w-full  justify-around'>
                              <label className='mx-4 cursor-pointer mt-2' htmlFor="name">Image Name</label>
                          <input required  value={editBanner.imageName} onChange={(e)=>setEditBanner({...editBanner,imageName:e.target.value})} placeholder={banner.imageName} className=' rounded-3xl p-3 border-1 border-gray w-1/2' type="text" id='name' />
                          </div>
                        <div className='flex flex-row flex-nowrap w-full  justify-between'>
                           <label className='cursor-pointer mt-2' htmlFor="source">Upload New Image</label>
                          <input required className=' w-1/2 rounded-3xl p-3 border-1 border-gray' onChange={async(e)=>{
                            const file=e.target.files?.[0]
                            if(!file) return
                            const compressedSrc=await compressAndConvertToBase64(file)
                            setEditBanner({...editBanner,src:compressedSrc})
 ...
 :null}
                  ...
                  <span><Button onClick={()=>handleDelete(banner.imageName)} className='!text-black !font-bold !text-md !lg:text-xl !px-4 !py-2 !rounded-2xl !border-1 !border-gray cursor-pointer' variant='ghost'>Delete</Button></span>
              </div>
  ...
  onPressedChange={()=>handleToggle(banner.imageName)}
